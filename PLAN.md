...
- 番号: ADR-**0016**（0014まで使用済み）、CR=**CHG-0018**（0016=候補テーブル固定ヘッダー〔別ブランチ・未マージ〕、0017=ウォッチリスト序列づけCRとして予約済み）
- **CHG-0016との整合に注意**: 未マージの`feat/chg0016-candidate-table-sticky-header`は、UC-012候補チェック画面のPER/PBRについて「チップに対応項目が無いため生の数値列として残す」前提で重複列を削除している。本CRでチップを追加するとこの前提が崩れるため、後からマージする側がUC-012の生PER/PBR列を除去する追随対応が必要（ADR-0016に記録済み）

### Files touched

**ドキュメント（Phase 0、本セッション、`feat/chg0018-undervalued-quality-buy-signal`ブランチ、mainから分岐）**: `docs/adr/ADR-0016-undervalued-quality-buy-signal.md`（新規、Status: Proposed）、`docs/product/use-cases.md`（UC-010業務ルール・出力表・UC-012の`criteria`項目数参照箇所）、`docs/architecture/data-model.md`（`buy_signals`/`watchlist_buy_signals`のenum定義・共通前提・初期パラメータ値・near バッファ・承認記録・変更履歴）、`docs/product/accuracy-improvement-backlog.md`（セクター別相対評価を後続候補として追記）、`docs/rcid/traceability-matrix.md`（F-010行・CHG-0018変更追跡行）、`PLAN.md`（本エントリ。300行に近づいたためCHG-0007エントリを`docs/history/plan-archive.md`へ退避）
