...
| id | bigint | NO | auto | 主キー |
| holding_snapshot_id | bigint | NO | - | `holding_snapshots.id` への参照 |
| signal_type | enum('rsi_oversold_rebound','macd_golden_cross','bollinger_oversold','week52_low_proximity','ma_deviation_oversold','volume_spike_rebound','peg_undervalued','per_undervalued') | NO | - | 買い増しシグナル種別（ADR-0007。`per_undervalued`は2026-09-19、CHG-0018／ADR-0015で追加） |
...
> **全シグナル共通の前提条件（2026-08-23追加。2026-09-19、CHG-0018／ADR-0015で(1)をOR条件に改訂、use-cases.md UC-010業務ルール参照）**: 長期低迷銘柄・個別要因での下落銘柄を誤って拾わないよう、8種いずれのシグナルも(1)(2)両方を満たした場合のみ検出する。
>   1. 次のいずれか（OR）: (a) 直近13週以内に`week52_high`の-15%以内に到達していたこと、または (b) `FundamentalHealthEvaluator::evaluate()` が `passed` であること（実データ検証で、52週高値から-35〜-48%乖離した財務健全な銘柄が複数存在することを確認し、これらを拾う目的で追加。ADR-0015）
>   2. `relative_strength_vs_market`が-5pt以上であること（変更なし）
>
> (1)(a)は`TechnicalIndicatorCalculator`が計算時に保持する週次価格系列から追加の外部データ取得なしに算出可能（`technical_indicators`には永続化せず、MA20乖離率と同様に判定時にアプリケーション層で計算する）。(1)(b)は`fundamental_indicators`から都度算出する（永続化しない）。(2)は既存の`relative_strength_vs_market`カラムをそのまま流用する。
> **PER単体シグナル `per_undervalued`（ADR-0015）**: `0 < PER ≤ 15.0`（`BuySignalDeterminationService::PER_UNDERVALUED_THRESHOLD`）を条件とする。下限ガード（`/review`指摘、2026-09-19追記）: US株のFinnhub `peTTM`は赤字企業で負値を返しうるため（`peg_ratio`と同じ構図、ADR-0012 D4）、下限なしだと赤字企業を誤って割安判定してしまう。PBRは会計恒等式 `PBR ≈ PER × ROE` により財務健全性フィルタ（ROE≥10%）とAND条件にすると実データで恒久的に0件になることを確認したため（ADR-0015 Context参照）、判定条件には使わず判定チェックリストの参考表示にのみ用いる。
> `signal_type`のENUM拡張は`.claude/rules/20-mysql.md`が定める「危険な操作（カラム型変更）」に該当する（MySQLの`ENUM`拡張は`ALTER TABLE ... MODIFY COLUMN`を要し、原理上テーブルロックを伴いうる。既存カラムへの追加のため`signals`のADR-0004と同種）。個人利用規模（`buy_signals`は数十〜数百行程度）のため実害は軽微だが、ADR-0015をもって正式な変更理由の記録とする。
...
| id | bigint | NO | auto | 主キー |
| holding_id | bigint | NO | - | `holdings.id` への参照 |
| signal_type | enum('rsi_oversold_rebound','macd_golden_cross','bollinger_oversold','week52_low_proximity','ma_deviation_oversold','volume_spike_rebound','peg_undervalued','per_undervalued') | NO | - | 押し目買いシグナル種別（`buy_signals` と同一。`per_undervalued`は2026-09-19、CHG-0018／ADR-0015で追加） |
...
| 買い増しシグナル8種の判定閾値（2026-09-19、CHG-0018で7→8） | RSI30以下からの反発／MACDゴールデンクロス／`bb_lower`割れ／52週安値+10%以内／MA20から-10%以上の下方乖離／出来高20週平均比1.5倍以上かつ前週比上昇／PEGレシオ1.0以下／**PER15.0以下（ADR-0015で追加）** | UC-010 | 叩き台のまま承認（ADR-0007。PER閾値はADR-0015、実データ検証で財務健全性passed24銘柄中4銘柄が該当することを確認済み）。`BuySignalDeterminationService`のGate4実装時に`/tdd`サイクルで確定 |
...
| 買い増しシグナル共通の前提条件（直前の好調さ・連れ安確認） | (1) 次のいずれか（OR、2026-09-19 ADR-0015で改訂）: (a)直近13週以内に`week52_high`の-15%以内到達、または (b)`FundamentalHealthEvaluator::evaluate()`が`passed`。(2)`relative_strength_vs_market`が-5pt以上（変更なし） | UC-010 | 叩き台のまま承認（2026-08-23、ADR-0007。(1)のOR条件はADR-0015、実データ検証で52週高値から-35〜-48%乖離した財務健全銘柄が前提(1)(a)単独では拾えないことを確認し追加）。`BuySignalDeterminationService`のGate4実装時に`/tdd`サイクルで確定 |
| PER単体の買い増しシグナル判定閾値（`per_undervalued`） | `0 < PER ≤ 15.0`（`BuySignalDeterminationService::PER_UNDERVALUED_THRESHOLD`）。下限ガードは`/review`指摘（US株Finnhub `peTTM`が赤字企業で負値を返しうるため、ADR-0012 D4のPEG下限ガードと同種）。PBRは独立条件に使わない（会計恒等式`PBR≈PER×ROE`により、財務健全性フィルタのROE≥10%要件とAND条件にすると実データで恒久的に0件になることを確認、ADR-0015 Context参照） | UC-010 | 叩き台のまま承認（2026-09-19、ADR-0015）。`BuySignalDeterminationService`のGate4実装時に`/tdd`サイクルで確定 |
| 判定チェックリストのPER・PBR表示（テクニカル7→9項目） | PER: 基準値≤15.0でmet/near/unmet色分け（`per_undervalued`と同一閾値）。PBR: 基準を持たない参考表示（実測値のみ、met/near/unmet色分けなしの中立ステータス`info`） | UC-010 | 叩き台のまま承認（2026-09-19、ADR-0015）。`SignalCriteriaEvaluator::evaluateBuy()`のGate4実装時に`/tdd`サイクルで確定 |
| 判定チェックリストの「あと一歩（`near`）」バッファ | 基準値 T に対し、\|T\| × 0.2 手前まで到達していれば「あと一歩」。「T 以上」条件は `実測 ≥ T − \|T\|×0.2`、「T 以下」条件は `実測 ≤ T + \|T\|×0.2`。T = 0 の項目（ボリンジャー乖離率・MACD差・成長率・相対力）は比率が定義できないため「達成／未達」の2値とする。実測値が未取得の項目は `unavailable` | UC-004/UC-010/UC-011 | 叩き台のまま承認（2026-08-29、CHG-0007）。`SignalCriteriaEvaluator`のGate4実装時に`/tdd`サイクルで確定。判定に用いる項目の基準値そのものは既存のシグナル判定閾値・財務健全性フィルタの値を流用し、本CRでは新設しない。**CHG-0012で財務グループが3→4項目になり、営業利益率（T=10、バッファ ±2.0 → 8.0〜10.0% が `near`）が加わる。営業利益率10%は本CRでの新設定数（他項目は既存流用のまま）**。**CHG-0018でUC-010のテクニカルグループが7→9項目になり、PER（T=15、バッファ±3.0 → 12.0〜15.0%が`near`）が加わる。PBRは基準を持たないため`met`/`near`/`unmet`の3値ではなく中立ステータス`info`（実測値のみ表示）とし、本バッファ計算の対象外とする（ADR-0015）** |
...
| 2026-09-19 | 買い増しシグナル共通前提の緩和とPER単体シグナルの追加を提案（CHG-0018／ADR-0015、UC-010、**Phase 0 ドキュメント先行**）。`buy_signals`/`watchlist_buy_signals`の`signal_type`ENUMに`per_undervalued`を追加（**本CR唯一のスキーマ変更・マイグレーション必要**、ENUM拡張のため`.claude/rules/20-mysql.md`の危険な操作に該当）。実データ検証（保有219銘柄、Context/ADR-0015参照）で当初案「PER≤15かつPBR≤1.0」が恒久的に0件になることを確認し、PER単体条件（PER≤15.0）に変更、PBRは判定チェックリストの参考表示のみに使用。共通前提(1)を「52週高値-15%以内到達」または「財務健全性`passed`」のOR条件に緩和。「買い増しシグナル8種の判定閾値」等の各行と`buy_signals`/`watchlist_buy_signals`カラム表を改訂。実装はGate3承認後に着手する | ADR-0015（CHG-0018） |
