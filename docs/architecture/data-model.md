...
| ma75_trend_rising | boolean | YES | null | 週足MA75の中期トレンドが上向きか（直近値が13週前の値を上回るか。ADR-0015 D5、UC-010の押し目買い事前条件に使用）。MA75算出に使う価格系列を13週分余分に遡れない場合（新規上場間もない等でデータ不足）はnull |
...
>
> **`ma75_trend_rising`列の追加（2026-09-19、CHG-0017／ADR-0015、Phase 0 ドキュメント先行）**: CHG-0012（`operating_margin`）以来のスキーマ変更。`TechnicalIndicatorCalculator`がMA75算出に使う週足終値シリーズを13週分余分に遡って同じ計算過程で導出するため、新規の外部データ取得は不要。既存の`relative_strength_vs_market`（13週騰落率比較）と同じ「呼び出し側が渡す価格系列から追加の時点を導出する」パターンを踏襲する。

> **低成長・財務健全銘柄の救済判定（2026-09-19改訂、ADR-0015 D1/D3）**: 当初`sector_classification_id`からの機械マッピング（スタイルタグ）で判定する設計だったが、実データで`sector_classification_id`がJP保有146銘柄中7銘柄しか埋まっておらず実効性がないと判明し撤回。**財務指標（ROE・配当利回り・自己資本比率・成長率）のみで判定する方式に変更し、新規DBカラムは追加しない**（セクター分類テーブルへの変更も無し）。
...
> **提案**（2026-09-06、CHG-0012・ADR-0011、Phase 0 ドキュメント先行）: 財務健全性フィルタの4条件目として `operating_margin`（営業利益率）を追加する。**本テーブルに `operating_margin decimal(10,4) nullable` を1列追加するマイグレーションが必要**（ADR-0009 以降で唯一のスキーマ変更）。US株は `stock/metric` の `operatingMarginTTM`（無ければ `operatingMarginAnnual`）を上記の `roe`/`revenue_growth` 等と同じくそのまま採用、JP株は `FundamentalIndicatorMapper` が既に取得済みの `net_sales`/`operating_profit` から `operating_profit ÷ net_sales × 100` で実測算出する（新規の外部API呼び出しなし）。両Mapperで `|営業利益率| > 999%` は null 化する。実装は Gate3 承認と F-011（`feat/f011-loss-review-list`）マージを待って着手。
...
| id | bigint | NO | auto | 主キー |
| holding_id | bigint | NO | - | `holdings.id` への参照 |
| fiscal_period | varchar(20) | NO | - | 決算期。実データはJ-Quants`DisclosedDate`由来の開示日文字列（例: `2026-05-01`）であり、四半期・本決算の区別はこの値だけでは判別できない（下記`period_type`追加の理由） |
...
| period_type | varchar(10) | YES | null | 決算期区分（`FY`/`1Q`/`2Q`/`3Q`）。J-Quants`CurPerType`をそのまま保存。2026-09-19追加（CHG-0017／ADR-0015 D2）。`JQuantsClient::fetchStatements()`のレスポンスには既に含まれていたが、追加前は`financial_statements`に永続化されていなかった（取得済みデータが保存時に捨てられていた） |
| fiscal_year_end | varchar(20) | YES | null | 会計年度末（J-Quants`CurFYEn`）。同一`fiscal_year_end`の重複開示行を除いて本決算（`period_type='FY'`）行を一意に特定するために使う。2026-09-19追加（CHG-0017／ADR-0015 D2）。`period_type`と同じ理由で追加 |
...
> `split_limit_suggestion`（分割指値提案）はシグナルではなく含み益率のみから機械的に算出できるため、テーブルには持たずアプリケーション層で都度計算する（UC-004業務ルール「固定ルールではなく例示」）。**初期パラメータ値（通常モード）**: 含み益+20%到達で1/3・+35%到達で1/3・残りはトレンド追従。含み益+20%未満の銘柄は本テーブルの対象外（`signals`行を作らない）。**高水準モード（2026-08-28追加、CHG-0006）**では表示・集計レイヤー（`ShowSignalListAction`/`ShowImportSummaryReportAction`）側の`TakeProfitThresholdEvaluator`が+100%/+150%地点に読み替えるが、`signals`行の作成条件（含み益+20%超でシグナル判定を実行）自体は変更しない。したがって`signals`テーブルの中身・件数はこのCRの影響を受けず、UC-010（買い増しタイミングレコメンド）の対象抽出（`whereDoesntHave('signals')`）にも影響しない。
...
| id | bigint | NO | auto | 主キー |
| holding_snapshot_id | bigint | NO | - | `holding_snapshots.id` への参照 |
| signal_type | enum('rsi_oversold_rebound','macd_golden_cross','bollinger_oversold','week52_low_proximity','ma_deviation_oversold','volume_spike_rebound','peg_undervalued','per_undervalued') | NO | - | 買い増しシグナル種別（ADR-0007。`per_undervalued`は2026-09-19、CHG-0018／ADR-0016で追加） |
...
> **全シグナル共通の前提条件（2026-08-23追加、2026-09-19改訂〔CHG-0018／ADR-0016で(1)をOR条件に改訂〕、2026-09-21改訂〔D4対セクターフォールバック・D5 MA75確認を追加〕、use-cases.md UC-010業務ルール参照）**: 長期低迷銘柄・個別要因での下落銘柄を誤って拾わないよう、8種いずれのシグナルも(1)(2)(3)すべてを満たした場合のみ検出する。
>   1. 次のいずれか（OR）: (a) 直近13週以内に`week52_high`の-15%以内に到達していたこと、または (b) `FundamentalHealthEvaluator::evaluate()` が `passed` であること（実データ検証で、52週高値から-35〜-48%乖離した財務健全な銘柄が複数存在することを確認し、これらを拾う目的で追加。ADR-0016）
>   2. `relative_strength_vs_sector`が非nullならその値、nullなら`relative_strength_vs_market`が-5pt以上であること（ADR-0015 D4）
>   3. `ma75_trend_rising`が`false`ではないこと（null＝88週未満のデータ不足はブロックしない。ADR-0015 D5、2026-09-21本人確認）
>
> (1)(a)は`TechnicalIndicatorCalculator`が計算時に保持する週次価格系列から追加の外部データ取得なしに算出可能（`technical_indicators`には永続化せず、MA20乖離率と同様に判定時にアプリケーション層で計算する）。(1)(b)は`fundamental_indicators`から都度算出する（永続化しない）。(2)は既存の対セクター・対市場カラムを優先順位付きで流用する。(3)は`ma75_trend_rising`の算出に88週分の週足データが必要という高いデータ要件があり、(1)(2)と同じ「nullは即ブロック」方針を取ると直近上場銘柄等で押し目買いシグナルが一律に出なくなる回帰リスクがあるため、「明確に下向き（`false`）と判定された場合のみブロックする」設計とした。
> **PER単体シグナル `per_undervalued`（ADR-0016）**: `0 < PER ≤ 15.0`（`BuySignalDeterminationService::PER_UNDERVALUED_THRESHOLD`）を条件とする。下限ガード（`/review`指摘、2026-09-19追記）: US株のFinnhub `peTTM`は赤字企業で負値を返しうるため（`peg_ratio`と同じ構図、ADR-0012 D4）、下限なしだと赤字企業を誤って割安判定してしまう。PBRは会計恒等式 `PBR ≈ PER × ROE` により財務健全性フィルタ（ROE≥10%）とAND条件にすると実データで恒久的に0件になることを確認したため（ADR-0016 Context参照）、判定条件には使わず判定チェックリストの参考表示にのみ用いる。
> `signal_type`のENUM拡張は`.claude/rules/20-mysql.md`が定める「危険な操作（カラム型変更）」に該当する（MySQLの`ENUM`拡張は`ALTER TABLE ... MODIFY COLUMN`を要し、原理上テーブルロックを伴いうる。既存カラムへの追加のため`signals`のADR-0004と同種）。個人利用規模（`buy_signals`は数十〜数百行程度）のため実害は軽微だが、ADR-0016をもって正式な変更理由の記録とする。
...
| id | bigint | NO | auto | 主キー |
| holding_id | bigint | NO | - | `holdings.id` への参照 |
| signal_type | enum('rsi_oversold_rebound','macd_golden_cross','bollinger_oversold','week52_low_proximity','ma_deviation_oversold','volume_spike_rebound','peg_undervalued','per_undervalued') | NO | - | 押し目買いシグナル種別（`buy_signals` と同一。`per_undervalued`は2026-09-19、CHG-0018／ADR-0016で追加） |
...
> **2026-09-17: 書き込み廃止（ADR-0014 D9-2）**。UC-009の上位10〜20件レコメンドがF-013（ポートフォリオ分類ダッシュボード）の分類俯瞰に置き換わったことに伴い、本テーブルおよび`import_summary_report_items`への書き込み（`ShowImportSummaryReportAction::persist()`・`ImportCsvAction`のプレースホルダー行作成）を廃止した。実装調査の結果、両テーブルは書き込み専用（読み返す機能が存在しない）だったため、廃止による機能影響はない。テーブル定義自体は本CRでは削除せず、以下は歴史的な記録として残す。
...
|---|---|---|---|
| 分割指値の閾値・比率（通常モード） | +20%地点で1/3、+35%地点で1/3、残りはトレンド追従 | UC-004 | 叩き台のまま承認 |
| 利確検討ラインの動的分岐（高水準モード） | 条件: 現在シグナル0件 かつ 財務健全性フィルタ（上記行）を満たす銘柄のみ。該当時: 対象抽出は含み益+150%超、分割指値は+100%地点で1/3・+150%地点で1/3・残りトレンド追従（非該当銘柄は通常モードのまま） | UC-004/UC-009 | 叩き台のまま承認（2026-08-28、CHG-0006）。`TakeProfitThresholdEvaluator`のGate4実装時に`/tdd`サイクルで確定 |
...
| 財務健全性フィルタ | 自己資本比率40%以上・ROE10%以上・成長率（売上高または営業利益成長率のいずれか）プラス・**営業利益率10%以上（CHG-0012／ADR-0011で4条件目として追加）** | UC-005/UC-008/UC-009/UC-010/UC-011 | **確定済み（営業利益率条件を除く）**（UC-008分2026-08-23`NewCandidateFinder`Gate4、UC-005分2026-08-23`SectorAllocationCalculator`Gate4で同一値をそのまま流用。成長率条件は2026-08-27、CHG-0005でUC-010と統一するため追加。`FundamentalHealthEvaluator`を`NewCandidateFinder`・`ShowImportSummaryReportAction`から共通利用する形に統合し、UC-005〔`ShowSectorDashboardAction`が`NewCandidateFinder`を直接利用〕にも自動的に反映される）。**営業利益率10%は叩き台承認（CHG-0012、2026-09-06）。`FundamentalHealthEvaluator`への4条件目追加のGate4実装時に`/tdd`サイクルで確定。8%案（実測: passed 27→25）と比較し10%案（実測: passed 27→22、境界9〜10%の3銘柄を切る）を採用した経緯はADR-0011参照。判定順序はROE/自己資本比率の基準割れ→営業利益率の基準割れ→営業利益率null時unavailable→成長率、で `/review`修正1（基準割れをデータ欠損より優先）の不変条件を踏襲する** |
...
| ~~サマリーレポートの件数区分~~ | ~~主要10件・補足10件（計20件）~~ | UC-009 | **廃止**（2026-09-17、ADR-0014 D9。上位10〜20件レコメンド自体がF-013の分類俯瞰に置き換わり件数区分の概念が無くなった） |
| ~~合成スコアの重み付け~~ | ~~利確検討・リバランス・新規投資候補の3種を横断する優先順位ロジック（未確定）~~ | UC-009 | **廃止**（2026-09-17、ADR-0014 D9-1・D11。ADR-0003 Superseded。分類俯瞰は透明マルチキーソート〔下記F-013行〕を用いるため非開示合成スコア自体が不要に） |
...
| 買い増しシグナル8種の判定閾値（2026-09-19、CHG-0018で7→8） | RSI30以下からの反発／MACDゴールデンクロス／`bb_lower`割れ／52週安値+10%以内／MA20から-10%以上の下方乖離／出来高20週平均比1.5倍以上かつ前週比上昇／PEGレシオ1.0以下／**PER15.0以下（ADR-0016で追加）** | UC-010 | 叩き台のまま承認（ADR-0007。PER閾値はADR-0016、実データ検証で財務健全性passed24銘柄中4銘柄が該当することを確認済み）。`BuySignalDeterminationService`のGate4実装時に`/tdd`サイクルで確定 |
| 買い増し用ファンダメンタルズ健全性フィルタ | 自己資本比率40%以上・ROE10%以上・成長率（売上高または営業利益）プラス・**営業利益率10%以上（CHG-0012／ADR-0011）** | UC-010 | 叩き台のまま承認（ADR-0007）。UC-009の財務健全性フィルタと同一値を用いる（上記「財務健全性フィルタ」行に統合。営業利益率条件もCHG-0012で同時に加わる）。`FundamentalHealthEvaluator`のGate4実装時に`/tdd`サイクルで確定 |
...
| 買い増しシグナル共通の前提条件（直前の好調さ・連れ安確認） | (1) 次のいずれか（OR、2026-09-19 ADR-0016で改訂）: (a)直近13週以内に`week52_high`の-15%以内到達、または (b)`FundamentalHealthEvaluator::evaluate()`が`passed`。(2)`relative_strength_vs_market`が-5pt以上（変更なし） | UC-010 | 叩き台のまま承認（2026-08-23、ADR-0007。(1)のOR条件はADR-0016、実データ検証で52週高値から-35〜-48%乖離した財務健全銘柄が前提(1)(a)単独では拾えないことを確認し追加）。`BuySignalDeterminationService`のGate4実装時に`/tdd`サイクルで確定 |
| PER単体の買い増しシグナル判定閾値（`per_undervalued`） | `0 < PER ≤ 15.0`（`BuySignalDeterminationService::PER_UNDERVALUED_THRESHOLD`）。下限ガードは`/review`指摘（US株Finnhub `peTTM`が赤字企業で負値を返しうるため、ADR-0012 D4のPEG下限ガードと同種）。PBRは独立条件に使わない（会計恒等式`PBR≈PER×ROE`により、財務健全性フィルタのROE≥10%要件とAND条件にすると実データで恒久的に0件になることを確認、ADR-0016 Context参照） | UC-010 | 叩き台のまま承認（2026-09-19、ADR-0016）。`BuySignalDeterminationService`のGate4実装時に`/tdd`サイクルで確定 |
| 判定チェックリストのPER・PBR表示（テクニカル7→9項目） | PER: 基準値≤15.0でmet/near/unmet色分け（`per_undervalued`と同一閾値）。PBR: 基準を持たない参考表示（実測値のみ、met/near/unmet色分けなしの中立ステータス`info`） | UC-010 | 叩き台のまま承認（2026-09-19、ADR-0016）。`SignalCriteriaEvaluator::evaluateBuy()`のGate4実装時に`/tdd`サイクルで確定 |
| 判定チェックリストの「あと一歩（`near`）」バッファ | 基準値 T に対し、\|T\| × 0.2 手前まで到達していれば「あと一歩」。「T 以上」条件は `実測 ≥ T − \|T\|×0.2`、「T 以下」条件は `実測 ≤ T + \|T\|×0.2`。T = 0 の項目（ボリンジャー乖離率・MACD差・成長率・相対力）は比率が定義できないため「達成／未達」の2値とする。実測値が未取得の項目は `unavailable` | UC-004/UC-010/UC-011 | 叩き台のまま承認（2026-08-29、CHG-0007）。`SignalCriteriaEvaluator`のGate4実装時に`/tdd`サイクルで確定。判定に用いる項目の基準値そのものは既存のシグナル判定閾値・財務健全性フィルタの値を流用し、本CRでは新設しない。**CHG-0012で財務グループが3→4項目になり、営業利益率（T=10、バッファ ±2.0 → 8.0〜10.0% が `near`）が加わる。営業利益率10%は本CRでの新設定数（他項目は既存流用のまま）**。**CHG-0018でUC-010のテクニカルグループが7→9項目になり、PER（T=15、バッファ±3.0 → 12.0〜15.0%が`near`）が加わる。PBRは基準を持たないため`met`/`near`/`unmet`の3値ではなく中立ステータス`info`（実測値のみ表示）とし、本バッファ計算の対象外とする（ADR-0016）** |
| 整理検討ライン（対象抽出＋チェックリスト①含み損率） | 含み益率が **-20%未満** で整理検討候補として対象。チェックリスト①「含み損率」も同閾値（≦-20%で `met`） | UC-011 | 叩き台のまま承認（2026-09-05、CHG-0010／ADR-0010）。`ShowLossReviewListAction`／`LossReviewThresholds` のGate4実装時に`/tdd`サイクルで確定。実データで件数が多すぎ／少なすぎる場合はここで調整する（`accuracy-improvement-backlog.md`「シグナル閾値のキャリブレーション」と同じ扱い） |
| 整理チェックリスト②「52週高値からの下落率」 | `(現在値 − week52_high) ÷ week52_high × 100` が **-30%以下** で `met`（＝直前の好調期から大きく毀損した） | UC-011 | 叩き台のまま承認（2026-09-05、CHG-0010）。UC-004の`week52_high_pullback`（-10%）とは別基準（あちらは利確過熱の反落、こちらは長期毀損の確認のため深い） |
| 整理チェックリスト⑦「押し目買いシグナル件数」 | 当該銘柄の `buy_signals`（UC-010）が **0件** で `met`（＝反発の兆しが出ていない）。基準値0のため「達成／未達」の2値 | UC-011 | 叩き台のまま承認（2026-09-05、CHG-0010） |
...
| `core_accumulation`バケツの並び順 | 評価額（`market_value`）が大きい順 | UC-013 | 叩き台のまま承認（2026-09-17、ADR-0014 D10） |
| セクター偏りサマリの並び順 | 配分率の超過幅（`allocation_rate` − 70%）が大きい順 | UC-013 | 叩き台のまま承認（2026-09-17、ADR-0014 D10） |
| `hold_watch`（要観察）の判定基準 | 財務`failed` または 相対力〔対市場〕が継続マイナス または 含み益率が整理検討ライン（-20%）の手前（叩き台: -15%〜-20%の範囲）まで悪化、のいずれかに該当 | UC-013 | 未確定（具体的なバッファ幅）。既存の評価値の組み合わせのみを使い新規の判定ロジック自体は追加しないが、「手前」の具体的な数値幅は`ClassifyHoldingsAction`のGate4実装時に`/tdd`サイクルで確定（2026-09-17、ADR-0014 D5） |
...
| 財務指標ベースの成長率救済経路（低成長でも財務健全な優良企業） | ROE≧15% かつ 自己資本比率≧50% を満たせば成長率の値を問わず`met`。閾値はNISA推奨基準（ROE15%/自己資本比率50%）をそのまま流用（新規閾値なし） | UC-004/UC-010/UC-011 | 叩き台のまま承認（2026-09-19改訂、CHG-0017／ADR-0015 D1）。**2026-09-19、Gate4レビュー時に再改訂**: 当初「配当利回り≧3%」もAND条件に含めていたが、無配当・低配当でも自社株買い等で資本効率を高める銘柄（特に米国株）を機械的に除外してしまうとの指摘で撤回。実測（保有218銘柄、成長率のみで`failed`/`unavailable`になる候補2件）: 配当必須なら救済0件、配当不問なら2件救済（名村造船所・東映アニメーション、いずれもROE/自己資本比率は優良）。配当利回りは判定に使わず理由サマリの参考表示のみ。セクター分類には依存しない全銘柄共通（JP/US問わず）。`FundamentalHealthEvaluator`のGate4実装時に`/tdd`サイクルで確定 |
| 成長率OR救済の算出期数 | `financial_statements`の直近3〜5期の平均成長率（売上高・営業利益いずれか）が>0%で`met`（全銘柄共通、セクター非依存） | UC-004/UC-010/UC-011 | 叩き台のまま承認（2026-09-19、CHG-0017／ADR-0015 D2）。3期・4期・5期のどれを採用するかは`FundamentalHealthEvaluator`のGate4実装時に実データで比較して確定。US株は`financial_statements`を保存しないため対象外（単年度判定のみ）。**2026-09-19追記**: 本行の前提として`financial_statements`に`period_type`/`fiscal_year_end`列（上記テーブル定義参照）が必要と判明し追加した（Gate4 Cycle2着手時。従来はFYか四半期かをDB上で判別できず算出不可能だった） |
| 低成長銘柄のPEG除外条件とPER/配当利回り絶対閾値 | 成長率〔売上高・営業利益の高い方〕が5%以下（暫定閾値）の銘柄はPEG判定を対象外とし、PER≦15 かつ 配当利回り≧3% で「割安」と判定 | UC-004/UC-010 | 叩き台のまま承認（2026-09-19改訂、CHG-0017／ADR-0015 D3）。発動条件をセクター分類ではなく成長率データに変更（実データで`sector_classification_id`がほぼ未整備と判明したため）。資料2の提案値をそのまま採用。実データで影響銘柄数を確認してから`FundamentalHealthEvaluator`／`SignalDeterminationService`／`BuySignalDeterminationService`のGate4実装時に`/tdd`サイクルで確定 |
| 週足MA75の中期トレンド判定 | 直近値が13週前の値を上回れば`ma75_trend_rising=true`（UC-010の押し目買い事前条件に追加） | UC-010 | 叩き台のまま承認（2026-09-19、CHG-0017／ADR-0015 D5）。13週は既存の相対力算出期間・買い増し前提条件の「直近13週」と揃えた値。`TechnicalIndicatorCalculator`のGate4実装時に`/tdd`サイクルで確定 |
...
**整理検討候補一覧の算出項目（`ShowLossReviewListAction`、UC-011・ADR-0010）**: いずれも表示レイヤーで既存カラムから都度算出し、永続化しない。DBスキーマ変更なし。

| 項目 | 計算式 | データ不足時 |
|---|---|---|
| `recovery_required_rate`（復帰に必要な上昇率、%） | 含み益率 `r`（`holding_snapshots.unrealized_gain_rate`、本UCの対象は常に負）に対し `(1 / (1 + r/100) − 1) × 100`。取得単価まで戻すのに必要な上昇率。例: r=-40% → +66.7% | `unrealized_gain_rate` は NOT NULL のため常に算出可能 |
| `portfolio_loss_share`（ポートフォリオ比、%） | `abs(holding_snapshots.unrealized_gain_amount) ÷ ポートフォリオ評価総額 × 100`。分母は既存 `PortfolioEvaluationCalculator::total()`（直近スナップショットの全 `holding_snapshots`、投資信託は単位補正込み）を流用 | 分母が0のとき `null`（0除算回避） |
| MA75乖離率（チェックリスト④） | `(現在値 − ma75) ÷ ma75 × 100`。`technical_indicators.ma75`（75件必要、不足時 `null`）を用いる。UC-010のMA20乖離率と同じ式で参照MAだけ長期の MA75 に差し替え（長期トレンドからの逸脱を見るため） | `ma75` が `null` のときチェックリスト④は `unavailable` |
| `continuous_holding_weeks` | `snapshots` を `snapshotted_at` 降順で並べ、対象 `holding_id` の `holding_snapshots` が直近から連続して存在する数を数える。最古まで連続なら `continuous_holding_weeks_is_truncated=true`。対象 `holding_id` 群の `holding_snapshots` は `whereIn` で1クエリ取得（N+1回避）。純粋計算は `ContinuousHoldingWeeksCalculator` に分離 | 直近スナップショットに存在する銘柄が対象のため最小1 |

**判定チェックリストの価格乖離項目における通貨単位の整合（UC-004/UC-010/UC-011 共通、2026-09-05 CHG-0010 で横断修正）**: 判定チェックリスト（`SignalCriteriaEvaluator`）が `percentDeviation(現在値, week52_high／week52_low／ma20／ma75／bb_*)` で計算する乖離率チップは、両辺の通貨単位を揃える必要がある。`holding_snapshots.current_price` は米国株では CSV 取込時に参考為替レートで**円換算済み**（UC-001 業務ルール）だが、`technical_indicators` の価格系カラムは価格系列（Yahoo Finance）由来で**現地通貨（USD）のまま**。両者を直接比較すると米国株の乖離率が約 fx 倍に膨れる。`SignalCriteriaEvaluator::indicatorComparablePrice(?現在値, ?fx_rate_used)`（新設 public static）が、`fx_rate_used` が非 null かつ正のとき `現在値 ÷ fx_rate_used` で USD に割り戻す（JP株は `fx_rate_used = null` のため素通し）。この換算は判定チェックリストに渡す `current_price` のみに適用し、評価額（`market_value`、CHG-0011）・分割指値の価格・`recovery_required_rate`・`portfolio_loss_share` など円建てで意味を持つ値には適用しない。CHG-0007（判定チェックリスト新設）時点から潜在していたバグで、CHG-0009（Finnhub の米国株データ投入）で顕在化、F-011 の実データ確認で発見した。シグナル判定ロジック（`SignalDeterminationService`／`BuySignalDeterminationService`）は USD 価格系列内で完結しているため影響を受けない。DBスキーマ変更なし。
...
| 2026-09-17 | minowaryo | 承認（Gate3、影響範囲確認のみ） | **ポートフォリオ分類ダッシュボード（UC-013、F-013、ADR-0014）Gate1/2最終確定にあわせGate3も確認**。第1段階は新規テーブル・カラム追加なし（`ClassifyHoldingsAction`はいずれも既存カラムの再投影）。`import_summary_reports`/`import_summary_report_items`への書き込みを廃止（テーブル定義は残置、ADR-0014 D9-2）。「保留・確定が必要な初期パラメータ値」表に`take_profit`/`hold`/`core_accumulation`バケツの並び順・セクター偏りサマリの並び順・`hold_watch`判定基準の5行を追記、UC-009の「サマリーレポートの件数区分」「合成スコアの重み付け」の2行を廃止（取り消し線）。ADR-0003はSuperseded | ADR-0014 |
| 2026-09-19 | minowaryo | 承認（Gate3） | バリュー/景気敏感銘柄向け判定ロジック分岐（CHG-0017／ADR-0015）。`technical_indicators` に **`ma75_trend_rising` boolean nullable を1列追加**（CHG-0012〔`operating_margin`〕以来のスキーマ変更。`after('relative_strength_vs_sector')`）。「保留・確定が必要な初期パラメータ値」表に財務指標ベースの成長率救済経路・成長率OR救済の算出期数・低成長銘柄のPEG除外条件とPER/配当利回り絶対閾値・週足MA75の中期トレンド判定の4行を追記。実装は`TechnicalIndicatorCalculator`／`FundamentalHealthEvaluator`／`SignalDeterminationService`／`BuySignalDeterminationService`のGate4実装時に`/tdd`サイクルで確定 | ADR-0015（CHG-0017） |
| 2026-09-19 | minowaryo | 承認（Gate3追加） | Cycle2（D2）着手にあたり`financial_statements`に`period_type`/`fiscal_year_end`の2列を追加する軽量migrationを承認（既存の`fiscal_period`だけではFY/四半期を判別できないと判明。詳細は上記テーブル定義・変更履歴参照）。新規の外部API呼び出しは不要 |
| 2026-09-19 | minowaryo | 承認（Gate4 Cycle1着手時改訂） | CHG-0017（ADR-0015）のD1・D3を改訂。実データで`sector_classification_id`がJP保有146銘柄中7銘柄しか埋まっておらず（CR発端の伊藤忠8001も未分類）、セクター分類ベースの「スタイル」判定を撤回し財務指標のみで判定する方式に変更（詳細はADR-0015参照）。上記「保留・確定が必要な初期パラメータ値」表の該当3行を改訂。`technical_indicators.ma75_trend_rising`の追加（D5）は変更なし | ADR-0015（CHG-0017） |
...
| 2026-09-05 | 整理検討（含み損）候補一覧を新規追加（CHG-0010、ADR-0010、UC-011）。DBスキーマ変更なし・表示レイヤー完結（CHG-0006と同方式）。「保留・確定が必要な初期パラメータ値」表に整理検討ライン(-20%)ほか6行、「分析ロジックの計算仕様」節に`recovery_required_rate`・`portfolio_loss_share`・MA75乖離率・`continuous_holding_weeks`の算出式を追記。新設クラス`ShowLossReviewListAction`/`LossReviewThresholds`/`ContinuousHoldingWeeksCalculator`（いずれも既存の`holding_snapshots`/`technical_indicators`/`fundamental_indicators`/`buy_signals`/`snapshots`の値のみ参照） | ADR-0010（CHG-0010） |
| 2026-09-06 | 整理検討テーブルの判定チェックリストを「赤の単一極性」に変更（ADR-0010 D6 改訂、CHG-0010）。財務健全性3項目の判定の向きを反転し「基準割れ（＝投資根拠の毀損）」を `met`（赤チップ）とする。閾値の値は既存流用のまま、反転は表示レイヤーのみ（`SignalCriteriaEvaluator::fundamentalRows()` に反転フラグ追加）。UC-004/UC-010 の既定挙動・配色は不変。「整理検討の財務健全性3項目」行を改訂。DBスキーマ変更なし | ADR-0010（CHG-0010） |
...
| 2026-09-17 | ポートフォリオ分類ダッシュボード（F-013／ADR-0014／UC-013）Gate1/2最終確定。DBスキーマ変更なし（既存カラムの再投影のみ）。`import_summary_reports`/`import_summary_report_items`への書き込みを廃止（UC-009の上位10〜20件レコメンドが分類俯瞰に置き換わったため、`ImportCsvAction`のプレースホルダー行作成も含め廃止。テーブル定義は残置）。ADR-0003（非開示合成スコア）はSuperseded。「保留・確定が必要な初期パラメータ値」表にバケツ内ソート順4行・`hold_watch`判定基準1行を追記、UC-009の件数区分・合成スコア重み付けの2行を廃止。`ShowSignalListAction`（UC-004）に並び順ロジックを1件追加（既存ロジックの改修は本CRを通してこれが唯一） | ADR-0014 |
| 2026-09-19 | 買い増しシグナル共通前提の緩和とPER単体シグナルの追加を提案（CHG-0018／ADR-0016、UC-010、**Phase 0 ドキュメント先行**）。`buy_signals`/`watchlist_buy_signals`の`signal_type`ENUMに`per_undervalued`を追加（**本CR唯一のスキーマ変更・マイグレーション必要**、ENUM拡張のため`.claude/rules/20-mysql.md`の危険な操作に該当）。実データ検証（保有219銘柄、Context/ADR-0016参照）で当初案「PER≤15かつPBR≤1.0」が恒久的に0件になることを確認し、PER単体条件（PER≤15.0）に変更、PBRは判定チェックリストの参考表示のみに使用。共通前提(1)を「52週高値-15%以内到達」または「財務健全性`passed`」のOR条件に緩和。「買い増しシグナル8種の判定閾値」等の各行と`buy_signals`/`watchlist_buy_signals`カラム表を改訂。実装はGate3承認後に着手し、2026-09-21のmainマージ時にADR番号0015→0016へ採番し直した（`.claude/rules/06-branch-coordination.md`参照） | ADR-0016（CHG-0018） |
| 2026-09-19 | バリュー/景気敏感銘柄向け判定ロジック分岐（CHG-0017／ADR-0015、**Phase 0 ドキュメント先行、Gate1/2承認済み**）。`technical_indicators`に`ma75_trend_rising` boolean nullableを1列追加するマイグレーションが必要（CHG-0012〔`operating_margin`〕以来のスキーマ変更）。当初は銘柄スタイル（growth/value_cyclical）を`sector_classification_id`の機械マッピングで導出する設計としたが、同日中のGate4 Cycle1着手時に撤回（下記エントリ参照）。「保留・確定が必要な初期パラメータ値」表に4行を追記。実装は`FundamentalHealthEvaluator`／`SignalDeterminationService`／`BuySignalDeterminationService`／`TechnicalIndicatorCalculator`のGate4実装時に`/tdd`サイクルで確定。Gate3承認待ち | ADR-0015（CHG-0017） |
| 2026-09-19 | CHG-0017（ADR-0015）のD1・D3を改訂（Gate4 Cycle1着手時）。実データ確認で`sector_classification_id`がJP保有146銘柄中7銘柄しか埋まっておらず（CRの発端である伊藤忠8001も未分類）、セクター分類ベースの「スタイル」判定は実効性がないと判明。業種名による二分自体も本人指摘（食品・建設の例）により妥当性が薄いと判断し撤回。**財務指標のみで判定する方式に変更**: D1は「ROE≧15%かつ配当利回り≧3%かつ自己資本比率≧50%」を満たせば成長率を問わず救済する経路（全銘柄・セクター非依存）に、D3は「PEG除外の発動条件」を「value_cyclical銘柄」から「成長率が低い（5%以下、暫定）銘柄」に変更。`ma75_trend_rising`列の追加（D5）・3〜5期平均成長率（D2）・対セクター相対力フォールバック（D4）は変更なし | ADR-0015（CHG-0017） |
| 2026-09-19 | D1の閾値をさらに改訂（Gate4レビュー時）。「配当利回り≧3%」をAND条件から削除し、D1＝「ROE≧15%かつ自己資本比率≧50%」のみに簡素化。無配当・低配当でも自社株買い等で資本効率を高める銘柄（特に米国株）を機械的に除外してしまうとの指摘（本人）を受け、実測比較（配当必須で救済0件／配当不問で2件救済）のうえ決定。D3（PER≤15かつ配当利回り≥3%）は別の判定目的のため変更なし | ADR-0015（CHG-0017） |
| 2026-09-19 | Cycle2（D2: 複数期平均成長率）着手にあたり`financial_statements`の前提を確認したところ、`fiscal_period`が開示日文字列のみでFY/四半期の区別がDB上でできないと判明（実データ8001で累積開示パターンを確認）。`JQuantsClient::fetchStatements()`が既に取得している`period_type`/`fiscal_year_end`を`financial_statements`に永続化していなかったことが原因。**`financial_statements`に`period_type` varchar(10) nullable・`fiscal_year_end` varchar(20) nullableの2列を追加**（新規の外部API呼び出しは不要）。テーブル定義・初期パラメータ表「成長率OR救済の算出期数」行を更新 | ADR-0015（CHG-0017） |
| 2026-09-19 | Cycle4a（D2配線）実装に伴い`fundamental_indicators`に`avg_revenue_growth`/`avg_operating_income_growth` decimal(10,4) nullableの2列を追加（`FundamentalIndicatorMapper::averageAnnualGrowth()`、直近4期のFY決算から3期分のYoYを平均、JP株限定）。`FetchExternalMarketDataAction`（保有銘柄）で先行配線し、`fundamental_indicators`カラム表を改訂。2回目の`/review`で`RefreshWatchlistMarketDataAction`（未保有ウォッチリスト銘柄）側にこの配線が漏れていることが判明したため、同一Cycleで追加修正（Red: `avg_revenue_growth`がnullのまま保存されるテストを追加→Green: `refreshHolding()`のJP分岐に同じ`averageAnnualGrowth()`呼び出しを追加）。あわせて`FundamentalIndicatorMapper::annualGrowth()`/`averageAnnualGrowth()`で重複していたFY絞り込み・重複排除・ソート処理を`annualStatementsDescending()`に共通化（`/review`指摘、挙動は変更なし） | ADR-0015（CHG-0017） |
