...
> **PER単体シグナル `per_undervalued`（ADR-0015）**: `PER ≤ 15.0`（`BuySignalDeterminationService::PER_UNDERVALUED_THRESHOLD`）のみを条件とする。PBRは会計恒等式 `PBR ≈ PER × ROE` により財務健全性フィルタ（ROE≥10%）とAND条件にすると実データで恒久的に0件になることを確認したため（ADR-0015 Context参照）、判定条件には使わず判定チェックリストの参考表示にのみ用いる。
...
| PER単体の買い増しシグナル判定閾値（`per_undervalued`） | `PER ≤ 15.0`（`BuySignalDeterminationService::PER_UNDERVALUED_THRESHOLD`）。PBRは独立条件に使わない（会計恒等式`PBR≈PER×ROE`により、財務健全性フィルタのROE≥10%要件とAND条件にすると実データで恒久的に0件になることを確認、ADR-0015 Context参照） | UC-010 | 叩き台のまま承認（2026-09-19、ADR-0015）。`BuySignalDeterminationService`のGate4実装時に`/tdd`サイクルで確定 |
