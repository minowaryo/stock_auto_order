...
- 現在のグローバルナビゲーション構成（5タブ）: 保有一覧（UC-002、UC-007の市場全体指標ウィジェットを内包）／利確シグナル（UC-004）／セクター配分（UC-005）／新規投資候補（UC-006・UC-008を統合）／CSV取込（UC-001）。銘柄詳細（UC-003）は一覧からのクリック遷移のみで、ナビゲーションタブは持たない
